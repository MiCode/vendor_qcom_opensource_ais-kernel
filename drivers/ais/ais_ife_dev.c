	...
	unsigned int cmd, void *arg)
{
	...
	if (!sd) {
		CAM_ERR(CAM_ISP, "AIS IFE subdev not init.");
		rc = -ENODEV;
		goto END;
	}
	p_ife_dev = v4l2_get_subdevdata(sd);
...
static int ais_ife_driver_cmd(struct ais_ife_dev *p_ife_dev, void *arg)
{
	...
	CAM_DBG(CAM_ISP, "IFE %d CMD %d", p_ife_dev->hw_idx, cmd->op_code);
	...
	switch (cmd->op_code) {
		...
		case AIS_IFE_RESET: {
			int tmp;
			CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_RESET",
				p_ife_dev->hw_idx);
		...
		case AIS_IFE_RESERVE: {
			...
			if (cmd->size != sizeof(rdi_init)) {
				...
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_RESERVE path %d",
					p_ife_dev->hw_idx, rdi_init.path);
		...
		case AIS_IFE_RELEASE: {
			...
			if (cmd->size != sizeof(rdi_deinit)) {
				...
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_RELEASE path %d",
					p_ife_dev->hw_idx, rdi_deinit.path);
		...
		case AIS_IFE_START: {
			...
			if (cmd->size != sizeof(rdi_start)) {
				...
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_START path %d",
					p_ife_dev->hw_idx, rdi_start.path);
		...
		case AIS_IFE_STOP: {
			...
			if (cmd->size != sizeof(rdi_stop)) {
				...
				int tmp;
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_STOP path %d",
					p_ife_dev->hw_idx, rdi_stop.path);
		...
		case AIS_IFE_PAUSE: {
			...
			if (cmd->size != sizeof(rdi_stop)) {
				...
				int tmp;
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_PAUSE path %d",
					p_ife_dev->hw_idx, rdi_stop.path);
		...
		case AIS_IFE_RESUME: {
			...
			if (cmd->size != sizeof(rdi_start)) {
				...
				CAM_INFO(CAM_ISP, "IFE %d cmd AIS_IFE_RESUME path %d",
					p_ife_dev->hw_idx, rdi_start.path);
