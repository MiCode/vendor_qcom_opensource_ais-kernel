	...
	struct ais_ife_csid_hw			*csid_hw)
{
	...
	/* Disable top csi inerrupts */
	val = cam_io_r_mb(soc_info->reg_map[0].mem_base + csid_reg->top_irq_reg_info->irq_reg_set->mask_reg_offset);
	val |= ~(csid_reg->csi2_reg->top_irq_mask);
	cam_io_w_mb(val,
			soc_info->reg_map[0].mem_base + csid_reg->top_irq_reg_info->irq_reg_set->mask_reg_offset);
	...
	void *reserve_args, uint32_t arg_size)
{
	...
	CAM_DBG(CAM_ISP, "CSID:%d reserve path :%d",
			csid_hw->hw_intf->hw_idx, rdi_cfg->path);
...
static int ais_csid_event_dispatch_process(void *priv, void *data)
{
	...
	switch (work_data->evt_type) {
		case AIS_IFE_MSG_CSID_ERROR:
			...
			CAM_INFO(CAM_ISP, "CSID%d send csid fatal error.", csid_hw->hw_intf->hw_idx);
...
static irqreturn_t ais_ife_csid_irq(int irq_num, void *data)
{
	...
	if (csid_hw->error_irq_count >
		...
		fatal_err_detected = true;
		csid_hw->error_irq_count = 0;
		CAM_ERR(CAM_ISP, "CSID%d: too many error detected.", csid_hw->hw_intf->hw_idx);
	} else if (warn_cnt) {
		uint64_t timestamp;
...
handle_fatal_error:
	spin_unlock_irqrestore(&csid_hw->lock_state, flags);
	if (fatal_err_detected && !csid_hw->fatal_err_detected) {
		uint64_t timestamp;
	...
	if (irq_status[CSID_IRQ_STATUS_BUFDONE]) {
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP, "CSID%d received buf done 0x%x.",
							csid_hw->hw_intf->hw_idx,
							buf_done.irq_status);
		csid_hw->vfe_hw->hw_ops.process_cmd(
				csid_hw->vfe_hw->hw_priv,
